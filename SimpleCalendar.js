...
var textYear = groupYear.add("statictext", undefined, undefined, {
  name: "textYear",
});
textYear.text = "Jahr";
...
var groupSelectEndSeperator = groupYear.add("group", undefined, {
  name: "groupSelectEndSeperator",
});
...
var textSelecetSeperator = groupSelectEndSeperator.add(
  "statictext",
  undefined,
  undefined,
  { name: "textSelecetSeperator" }
);
textSelecetSeperator.text = "Endseperator";

var endSeperator = groupSelectEndSeperator.add(
  "dropdownlist",
  undefined,
  undefined,
  { name: "endSeperator", items: dialogSelectSeperator }
);
endSeperator.selection = 3;
endSeperator.preferredSize.width = 180;

var textMonthSeperator = groupSelectEndSeperator.add(
  "statictext",
  undefined,
  undefined,
  {
    name: "textMonthSeperator",
  }
);
textMonthSeperator.text = "Seperator für Monatsende";

var monthSeperator = groupSelectEndSeperator.add(
  "dropdownlist",
  undefined,
  undefined,
  {
    name: "monthSeperator",
    items: dialogSelectSeperator,
  }
);
monthSeperator.selection = 0;
monthSeperator.preferredSize.width = 180;
...
var buttonClose = groupButton.add("button", undefined, "Abbrechen", {
  name: "buttonClose",
});
buttonClose.text = "Abbrechen";
buttonClose.onClick = function () {
  newDialog.close(false);
};
...
buttonRun.onClick = function () {
  ...
  if (
    inputYear.text.length != 4 ||
    inputYear.text <= 1970 ||
    inputYear.text >= 2100 ||
    isNaN(inputYear.text)
  ) {
    alert(
      "Überprüfe die Angaben im Feld 'Jahr'\n Gültige Werte: >1970 <2100, numerische Werte"
    );
    exit();
  }
  ...
  for (var i = 0; i < maingroup.children.length; i++) {
    selectionDate += myList.push(
      dateFormatList[maingroup.children[i].selectionFormat.selection.index][1] +
        "##" +
        seperatorList[
          maingroup.children[i].selectionSeperator.selection.index
        ][1]
    );
  }
...
if (app.selection[0] instanceof TextFrame) {
  ...
  alert(
    "Es wurde ein Rechteckrahmen ausgewählt!\rBitte einen Textrahmen auswählen"
  );
  exit();
}
  ...
  for (var i = 0; i < DaysOfYear(daysYear); i++) {
    ...
    for (var y = 0; y < myList.length; y++) {
      ...
      if (
        firstDay.getDate() == 1 &&
        firstDay.getMonth() > 0 &&
        firstDay.getMonth() <= 11
      ) {
        var dayOne = seperatorList[monthSeperator.selection.index][1];
      } else {
        var dayOne = "";
      }
    ...
    contentOut[i] =
      dayOne +
      dateOut.join("") +
      seperatorList[endSeperator.selection.index][1];
  }

  monthSeperator.selection;
...
function DaysOfYear(year) {
  ...
  return Math.round(
    Math.abs(firstDayYear - lastDayYear) / (1000 * 60 * 60 * 24) + 1
  );
}
...
function add_row(maingroup, out) {
  ...
  group.selectionFormat = group.add("dropdownlist", undefined, undefined, {
    name: "selectionFormat",
    items: dialogDateFormatList,
  });
  group.selectionFormat.selection = 0;
  group.selectionFormat.preferredSize.width = 200;

  group.selectionSeperator = group.add("dropdownlist", undefined, undefined, {
    name: "selectionSeperator",
    items: dialogSelectSeperator,
  });
